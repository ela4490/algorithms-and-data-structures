...
class Solution {
    ...
    public int[][] floodFill(int[][] image, int sr, int sc, int newColor) {
        ...
        dfs(image, source, sr, sc, newColor, row, col);
        
        return image;
    }
    
    private void dfs(int[][] image, int source, int sr, int sc, int newColor, int row, int col) {
        ...
        dfs(image, source, sr - 1, sc, newColor, row, col);
        dfs(image, source, sr + 1, sc, newColor, row, col);
        dfs(image, source, sr, sc - 1, newColor, row, col);
        dfs(image, source, sr, sc + 1, newColor, row, col);
    }
}